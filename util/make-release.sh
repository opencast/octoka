...
GIT_REMOTE="git@github.com:opencast/octoka.git"
...
old_version=$(sed -n -E 's/^version = "([^"]+)"$/\1/p' Cargo.toml)
...
echo "Ready. Will now adjust 'Cargo.toml' and 'Cargo.lock'. Ok?"
...
sed -i="" -E 's/^version = "[^"]+.0"$/version = "'"$major.$minor"'.0"/' Cargo.toml
sed -i -e \
    '/name = "octoka"/,/^version =.*$/ s/^version =.*$/version = "'"$major.$minor"'.0"/' \
    Cargo.lock
